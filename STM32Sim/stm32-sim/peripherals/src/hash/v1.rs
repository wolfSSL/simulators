...
//!   0x028..        digest size dependent regions, scratch
//!   0x310 HR0..HR7  (extended hash result, 8 words for SHA-256)
//!
//! ALGO selection on H7: bits {18, 7} of CR — 00 SHA-1, 01 MD5,
//! 10 SHA-224, 11 SHA-256.
...
pub enum AlgoLayout {
    /// H7 family: ALGO[0] at bit 7, ALGO[1] at bit 18.
    H7,
    /// U5 family: ALGO[0] at bit 17, ALGO[1] at bit 18.
    U5,
}
...
impl AlgoLayout {
    ...
    #[allow(dead_code)]
    fn algo_mask(self) -> u32 {
        (1u32 << self.lo_bit()) | (1u32 << self.hi_bit())
    }
}
...
impl HashV1 {
    ...
    fn parse_algo(&self) -> Algo {
        let lo_bit = 1u32 << self.layout.lo_bit();
        let hi_bit = 1u32 << self.layout.hi_bit();
