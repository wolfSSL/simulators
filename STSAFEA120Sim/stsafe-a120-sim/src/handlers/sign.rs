...
pub fn handle(device: &mut Device, body: &[u8]) -> Vec<u8> {
    ...
    let Some(slot_entry) = device.ecc_slots.get_mut(&slot) else {
    ...
    // wolfSSL pre-hashes the message and passes a 32-byte digest. Real
    // STSAFE silicon does the hashing internally; the simulator accepts
    // whatever the caller sends and signs it via PrehashSigner with a
    // length-adjusted digest (truncate or zero-pad on the high end to
    // match the curve's scalar size, which mirrors how ECDSA accepts
    // arbitrary-length digests per FIPS 186-5 6.4.1).
    let mut digest = [0u8; 32];
    let take = msg.len().min(32);
    digest[..take].copy_from_slice(&msg[..take]);
    let signature: Signature = match signing.sign_prehash(&digest) {
        Ok(s) => s,
    ...
    if slot_entry.usage_limit != 0 {
        slot_entry.used = slot_entry.used.saturating_add(1);
    }
