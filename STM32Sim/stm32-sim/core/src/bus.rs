...
/// Returns true if MMIO tracing is requested via the
/// `STM32_SIM_TRACE_MMIO` environment variable. Cached on first call
/// so the env lookup happens once per process. Set the variable to
/// `1`, `true`, or any non-empty value other than `0` / `false` to
/// enable.
fn trace_enabled() -> bool {
    static FLAG: OnceLock<bool> = OnceLock::new();
    *FLAG.get_or_init(|| match std::env::var("STM32_SIM_TRACE_MMIO") {
        Ok(v) => {
...
impl Bus {
    ...
    pub fn dispatch_read(&self, addr: u64, size: u8) -> u32 {
        for r in &self.regions {
            if addr >= r.base && addr < r.base + r.size {
                ...
                if trace_enabled() {
                    eprintln!(
                        "[mmio] R {:>6}+0x{:03x} (0x{:08x}) sz={} -> 0x{:08x}",
    ...
    pub fn dispatch_write(&self, addr: u64, size: u8, value: u32) {
        for r in &self.regions {
            if addr >= r.base && addr < r.base + r.size {
                let off = (addr - r.base) as u32;
                if trace_enabled() {
                    eprintln!(
                        "[mmio] W {:>6}+0x{:03x} (0x{:08x}) sz={} <- 0x{:08x}",
                        r.name, off, addr, size, value
                    );
                }
                match r.peripheral.lock() {
                    Ok(mut g) => g.write(off, size, value),
    ...
    /// 4 KiB-aligned pages this bus occupies. Used by the CPU to
    /// register Unicorn MMIO callbacks; Unicorn requires page-aligned
    /// MMIO mappings.
