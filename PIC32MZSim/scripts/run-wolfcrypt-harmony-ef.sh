...
#
# Build wolfSSL + the Microchip Harmony 3 crypto driver for PIC32MZ EF,
# link the wolfcrypt-test-harmony-ef firmware, and run it through the
...
WOLFSSL_SRC=/tmp/wolfssl-src-harmony-ef
WOLFSSL_BUILD=/opt/wolfssl-build-harmony-ef
HARMONY_BUILD=/opt/harmony-build-ef
FW=/app/firmware/wolfcrypt-test-harmony-ef
...
# Compile Harmony crypto driver objects we need (AES, SHA, RNG, HMAC).
# Harmony 3 ships per-algo source files; compile the PIC32MZ-targeted
# subset into a static library.
...
if [ ${#HARMONY_OBJS[@]} -gt 0 ]; then
    ...
    # Empty archive so the linker has something to consume; this lets
    # the run still produce a useful pass/fail signal if the Harmony
    # source layout changed upstream.
